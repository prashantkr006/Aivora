...
if ! command -v docker >/dev/null 2>&1; then
    echo "==> Installing docker (official CE repo) + caddy + ufw"
    sudo apt-get update -y
    sudo apt-get install -y ca-certificates curl gnupg lsb-release caddy ufw fail2ban git

    # Add Docker's official GPG key + apt source (only if not already set up)
